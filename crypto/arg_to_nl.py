...
class RipioBroker(Broker):
    ...
    def sync_exchange_rates(self):
        ...
        self.add_exchange_rate(Operation.SELL, ExchangeRate(Currency.BTC, Currency.ARS,
...
class QubitBroker(Broker):
    name = 'qubit'

    def __init__(self):
        super(QubitBroker, self).__init__(url='https://www.qubit.com.ar/c_value')

    def sync_exchange_rates(self):
        exchange_rate_data = requests.get(self.url).json()
        self.add_exchange_rate(Operation.SELL, ExchangeRate(Currency.BTC, Currency.ARS,
                                                            float(exchange_rate_data['BTC'][2])))
...
class BitonicBroker(Broker):
    ...
    def sync_exchange_rates(self):
        exchange_rate_data = requests.get(self.url).json()
        self.add_exchange_rate(Operation.BUY, ExchangeRate(Currency.BTC, Currency.EUR,
...
class BitvavoBroker(Broker):
    ...
    def __init__(self):
        super(BitvavoBroker, self).__init__(url='https://api.bitvavo.com/v2/ticker/book')

    def sync_exchange_rates(self):
        exchange_rate_data = requests.get(self.url).json()
        btc_eur_rate = next(rate for rate in exchange_rate_data if rate['market'] == 'BTC-EUR')

        self.add_exchange_rate(Operation.BUY, ExchangeRate(Currency.BTC, Currency.EUR,
                                                           float(btc_eur_rate['bid'])))
...
class Client:
    def get_btc_price(self, from_broker, in_currency=Currency.ARS):
        broker = Broker.create(name=from_broker.value)
        exchange_rate = broker.exchange_rate(Operation.SELL, from_currency=Currency.BTC, to_currency=Currency.ARS)
        return exchange_rate
    ...
    def send_amount(self, pesos, broker_arg=Brokers.RIPIO, broker_nl=Brokers.BITONIC):
        ...
        return {'EUR': round(receive_eur, 2), 'BTC': round(amount_of_btc, 8), 'ARS/EUR': pesos / receive_eur,
    ...
    def receive_amount(self, euros, broker_arg=Brokers.RIPIO, broker_nl=Brokers.BITONIC):
        ...
        return {'ARS': round(cost_ars, 2), 'BTC': round(amount_of_btc, 8), 'ARS/EUR': cost_ars / euros,
